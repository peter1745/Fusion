...
namespace Fusion {
	...
	VulkanGraphicsPipeline::VulkanGraphicsPipeline(const GraphicsPipelineInfo& InCreateInfo)
	    : m_CreateInfo(InCreateInfo)
	{
		auto Device = GraphicsContext::Get<VulkanContext>()->GetDevice().As<VulkanDevice>();

		auto VulkanPipelineShader = InCreateInfo.PipelineShader.As<VulkanShader>();

		std::vector<VkPipelineShaderStageCreateInfo> ShaderStageCreateInfos;
		std::vector<VkPushConstantRange> PushConstantRanges;

		const auto& ShaderModules = VulkanPipelineShader->GetShaderModules();

		std::vector<VkVertexInputBindingDescription> InputBindings;
		std::vector<VkVertexInputAttributeDescription> InputAttributes;

		for (const auto& [ShaderStage, ReflectionData] : VulkanPipelineShader->GetReflectedModules())
		{
		...
		vkCreatePipelineLayout(Device->GetLogicalDevice(), &PipelineLayoutCreateInfo, nullptr, &m_Layout);
		...
		vkCreateGraphicsPipelines(Device->GetLogicalDevice(), VK_NULL_HANDLE, 1, &PipelineCreateInfo, nullptr, &m_Pipeline);
	}

	void VulkanGraphicsPipeline::Bind(CommandList* InCmdList)
	{
		auto CommandBuffer = dynamic_cast<VulkanCommandList*>(InCmdList)->GetBuffer();
